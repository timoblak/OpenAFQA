...
# Where to store the computed feature vectors
FV_LOCATION = "sd302e_fv/"
# Initialize minutiae detector
minutiae_detect = FJFXWrapper(path_library="./libFJFX.so")
...
for im_num, im_name in enumerate(fingermark_ids):
    ...
    image = cv2.imread(fingermark_path, 0)
    t0 = time()
    ...
    if PRESET_PPI:
        ...
        ppi = int(im_name.split("\\")[-1].split("_")[6].replace("PPI", ""))
    ...
    print("DONE. Compute time: ", time() - t0)
    print("-----------------------------")
    ...
    if np.any(np.isnan(feature_vector)):
        print("ValueError: features in feature vector are nan values. ")
        exit()

    with open(FV_LOCATION + im_name.split(".")[0] + ".pkl", "wb") as handle:
